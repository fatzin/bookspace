...
import { TypeOrmBookingRepository } from "../../domain/Repository/TypeOrmBookingRepository";
import { TypeOrmPropertyRepository } from "../../domain/Repository/TypeOrmPropertyRepository";
import { TypeOrmUserRepository } from "../../domain/Repository/TypeOrmUserRepository";
import { BookingEntity } from "../persistence/Entity/BookingEntity";
import { PropertyEntity } from "../persistence/Entity/PropertyEntity";
...
beforeAll(async () => {
  ...
  app.post("/bookings", (req, res, next) => {
    bookingController.create(req, res).catch((err: any) => next(err));
  });

  app.post("/bookings/:id/cancel", (req, res, next) => {
    bookingController.cancel(req, res).catch((err: any) => next(err));
  });
});
